...
public class PolyNavAgent : MonoBehaviour{
	...
	void LateUpdate(){
		...
		if (rotateTransform){
            ...
            // david trebicky - unity editor says never used - so commenting out
            //float newZ0 = Mathf.MoveTowardsAngle(transform.localEulerAngles.z, 0, 5.0f * Time.deltaTime);


            //if (remainingDistance < slowingDistance){
            //    transform.localEulerAngles = new Vector3(transform.localEulerAngles.x, transform.localEulerAngles.y, newZ0);
            //}
            //else {
